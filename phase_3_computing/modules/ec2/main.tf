...
module "ec2_instance" {
  ...
  # ============================================================================
  # User Data - Cloud-init으로 nginx 설치 및 설정 (하드코딩)
  # ============================================================================
  # 설계 의도:
  # - 모든 인스턴스에 nginx 자동 설치 및 시작
  # - 서비스 정보를 HTTP 응답으로 반환 (서비스군명, 인스턴스 ID, AZ, IP)
  # - ALB health check 응답용 (HTTP 80 포트)
  # ============================================================================
  user_data_base64 = base64encode(templatefile("${path.module}/files/user_data.tftpl", {
    service_name = var.config.name
  }))
